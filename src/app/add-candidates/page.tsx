...
import { getContractWithSigner, isAdmin } from "@/utils/ethers";
...
export default function AddCandidates() {
  ...
  async function handleCreateVoting() {
    try {
      ...
      await tx.wait();
      setSuccess("Voting berhasil dibuat!");
  ...
  async function handleRegister() {
    try {
      const contract = await getContractWithSigner();
      const tx = await contract.registerVoter(1, voterAddress); // Asumsikan votingId = 1
      await tx.wait();
      setSuccess("Pemilih berhasil didaftarkan!");
      setVoterAddress("");
    } catch (err) {
      console.error(err);
      setError("Gagal mendaftarkan pemilih.");
    }
  }
